...
Shader::Shader(std::string vert_path, std::string frag_path) {
    read_files(vert_path,frag_path);
}


void Shader::read_files(std::string vert_path, std::string frag_path) {
    ...
    if(!shader_status) {
        ...
        printf("%s:%d Failed to compile vertex shader %s",__FILE__,__LINE__,shader_log);
        glfwTerminate();
        exit(-1);
    }
    ...
    if(!shader_status) {
        ...
        printf("%s:%d Failed to compile fragment shader %s",__FILE__,__LINE__,shader_log);
        glfwTerminate();
        exit(-1);
    }
    ...
    if(!shader_status) {
        ...
        printf("%s:%d Failed to link shader program %s",__FILE__,__LINE__,shader_log);
        glfwTerminate();
        exit(-1);
    }
